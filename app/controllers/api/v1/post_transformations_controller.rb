class Api::V1::PostTransformationsController < ApplicationController
  before_action :authenticate_token!
  before_action :set_post

  def show
    transform = find_transform
    if transform
      render json: {
        status: 200,
        content: {
          id: transform.id,
          content: transform.content,
          transform_type: transform.transform_type,
          created_at: transform.created_at
        }
      }
    else
      render json: { status: 404, message: 'No transformation found', content: {} }
    end
  end

  def create
    transform = find_transform || build_transform
    transform.content = params[:content][:body]
    transform.transform_type = params[:transform_type] || 'freeform'

    if transform.save
      render json: {
        status: 200,
        message: 'Page temporarily edited for you.',
        content: {
          id: transform.id,
          post_id: @post.id,
          title: @post.title,
          content: transform.content
        }
      }
    else
      render json: { status: 422, message: transform.errors.full_messages }
    end
  end

  def update
    transform = find_transform || build_transform
    transform.content = params[:content][:body]
    transform.transform_type = params[:transform_type] || 'freeform'

    if transform.save
      render json: {
        status: 200,
        message: 'Page temporarily edited for you.',
        content: {
          id: transform.id,
          post_id: @post.id,
          title: @post.title,
          content: transform.content
        }
      }
    else
      render json: { status: 422, message: transform.errors.full_messages }
    end
  end

  def destroy
    transform = find_transform
    if transform
      transform.destroy
      render json: { status: 200, message: 'Transformation removed. Page restored.' }
    else
      render json: { status: 404, message: 'No transformation found' }
    end
  end

  private

  def set_post
    @post = Post.find(params[:id])
  rescue ActiveRecord::RecordNotFound
    render json: { status: 404, message: 'Post not found' }
  end

  def find_transform
    if @current_user
      PostTransformation.for_user(@current_user).for_post(@post).last
    elsif @guest
      PostTransformation.for_guest(@guest).for_post(@post).last
    end
  end

  def build_transform
    if @current_user
      PostTransformation.new(user: @current_user, post: @post)
    elsif @guest
      PostTransformation.new(guest: @guest, post: @post)
    end
  end

  def authenticate_token!
    # Try user token from Authorization header
    auth_header = request.headers['Authorization']
    token = auth_header&.match(/^Bearer (.*)$/)&.[](1)
    @current_user = User.find_by(uuid: token) || User.find_by(uuid: request.headers['token'])

    # Try guest token
    unless @current_user
      @guest = Guest.find_by(uuid: token) || Guest.find_by(uuid: request.headers['token'])
    end

    render json: { status: 401, message: 'Authentication required' } unless @current_user || @guest
  end
end
